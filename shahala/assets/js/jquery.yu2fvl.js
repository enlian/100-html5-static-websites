  ...
  var win = $(w),
    ...
    getYTvid = function (url) {
      if (/youtu\.be/.test(url)) {
        ...
        /youtube.com|youtuberepeater.com|listenonrepeat.com/.test(url)
      ) {
  ...
  $.fn.yu2fvl = function (options) {
    ...
    if (settings.vid !== false) {
      ...
      return this.each(function () {
        var self = $(this);
        createLightbox(self, getYTvid(self.attr("href")));
      });
    }
    function createLightbox(btn, vid) {
      ...
      win.on("resize", resizeVideo).trigger("resize");
      ...
      function openVideo() {
        ...
        lightbox.stop().fadeIn("fast");
      }
      function closeVideo() {
        overlay.stop().fadeOut("fast");
        lightbox.stop().fadeOut("fast");
      }
    }
  };
